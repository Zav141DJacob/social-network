...
let signupFields = ['Nickname', 'Email', 'Password', 'ConfirmPassword', 'FirstName', 'LastName', 'Age', 'Bio', "Avatar"]

function InputField({id, name, submitId,placeholder, onClick, onKeyDown, good, password, error, focus, setUserCreator, userCreator}) {

  let elemId = good ? id + '-good' : id
  let errorId = good ? 'error-good' : 'error'
  let type = password ? 'password' : placeholder == 'Age' ? 'date' : placeholder === 'Avatar' ? 'file' : 'text'
...
export default function Login() {
  ...
  function handleSubmit(e) {
    ...
    switch (e.target.id) {
      case "planePath": {
        submitField = e.target.parentElement.parentElement.parentElement.previousElementSibling
        break
      }
      case "planeSVG": {
        submitField = e.target.parentElement.parentElement.previousElementSibling
        break
      }
      case "plane": {
  ...
  return (
    <div>
      <div className='login-module'>
        <div id='topbar'>
          <h1>LOG IN</h1>
          {error?.length > 0 ? <h3 style={{color: 'red'}}>Login failed. Please try again</h3> : <h3>Fill in all the information</h3>}
        </div>
        <div id='bottombar'>
          {errorFields && errorFields.map((bang, i) => {
